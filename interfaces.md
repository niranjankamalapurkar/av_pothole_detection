...
* Rationale: The World Model Builder's own fusion of the Live and Prior layers (interfaces #1, #2), per grid cell — combining two depth-grid patches into one fused patch is elementwise matrix fusion (e.g., Bayesian/evidential combination per cell), the same general-purpose mechanism the builder already applies to its other layers, not a bolt-on for this feature. patch_center_lat/lon here is absolute, not the grid's (typically ego-relative) internal representation — the World Model Builder is assumed to perform that local-to-global conversion internally, using its own Localization access (Baseline Dependency E), since it already needs a global reference to correlate against HD maps for its other functions. This is an assumption about baseline behavior this feature cannot verify — if it does not hold, the Reporter would need its own direct Localization access after all. Cloud-side fleet fusion (block_diagram.md Section 5) depends on this: the Healing Engine cannot correlate observations across vehicles without each one carrying an absolute location. This is the only place fusion happens; there is no upstream object-level matching step to define. fused_depth_values follow the same sign convention as interface #1 (negative = depression).

------------------------------
## 4. Vehicle Speed Calculator → Pothole Observation Reporter

* Protocol: CAN-FD
* Payload: timestamp, vehicle_speed_mph, vehicle_speed_health_state (NOMINAL | DEGRADED | FAULTED)
* Rationale: A feature-defined consumption of a baseline signal — the Vehicle Speed Calculator itself remains baseline vehicle architecture (Baseline Dependency A, feeding Path Planner separately); this feature defines its own, additional consumption for the Reporter's purposes, the same pattern used elsewhere in this design for baseline signals this feature needs but does not own. Used by pothole_observation_reporter_non_functional_requirements.md POR-NFR-01 to compute the deduplication suppression window (perception coverage radius ÷ current vehicle speed). vehicle_speed_health_state is included because that calculation, and the corresponding requirement to suppress reporting when the input is untrustworthy, both depend on knowing whether the speed reading itself is valid — Baseline Dependency A does not itself specify a health/validity signal for vehicle speed, so this feature cannot assume one exists elsewhere; if the Vehicle Speed Calculator's actual native output has no equivalent signal, this interface will need to be revisited once that is confirmed. Named consistently with perception_health_state's convention (NOMINAL | DEGRADED | FAULTED) rather than introducing a new enum style.

------------------------------
## 5. Pothole Observation Reporter → Connectivity Manager (Telemetry)
